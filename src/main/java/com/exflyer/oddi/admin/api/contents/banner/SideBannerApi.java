...
@Slf4j
@RestController
public class SideBannerApi {

    @Autowired
    private BannerService bannerService;

    @ApiOperation(value = "광고판 우측 배너관리 사용 코드", notes = "광고판 우측 배너관리 사용 코드 목록 조회 API 입니다. ")
    @GetMapping(path = "/side-banner/search-codes")
    @MenuValidApi(menuCode = "CNT002", operation = CrudOperation.READ)
    ...
    @ApiOperation(value = "광고판 우측 배너관리 리스트 조회", notes = "광고판 우측 배너관리 리스트 조회 API 입니다. ")
    @GetMapping(path = "/side-banner/search")
    @MenuValidApi(menuCode = "CNT002", operation = CrudOperation.READ)
    public ApiResponseDto<PagingResult<SearchBannerResult>> find(@Validated SearchBanner searchReq) throws ApiException {
        ...
        searchReq.setBannerLocationType(BannerCodes.BANNER_SIDE_LOCATION.getCode());
    ...
    @ApiOperation(value = "광고판 우측 배너관리 상세 조회", notes = "광고판 우측 배너관리 상세 조회 API 입니다. ")
    @GetMapping(path = "/side-banner/{seq}")
    @MenuValidApi(menuCode = "CNT002", operation = CrudOperation.READ)
    ...
    @ApiOperation(value = "광고판 우측 배너관리 등록", notes = "광고판 우측 배너관리 등록 API 입니다. ")
    @PostMapping(path = "/side-banner")
    @MenuValidApi(menuCode = "CNT002", operation = CrudOperation.READ)
    public ApiResponseDto<Banner>add(@Validated @RequestBody BannerRegReq regReq, AdminAuth adminAuth)
        ...
        regReq.setLocationCode(BannerCodes.BANNER_SIDE_LOCATION.getCode());
        regReq.setRegId(adminAuth.getId());
        bannerService.save(regReq);
        return new ApiResponseDto(ApiResponseCodes.SUCCESS);
    }

    @ApiOperation(value = "광고판 우측 배너관리 수정", notes = "광고판 우측 배너관리 수정 API 입니다. ")
    @PutMapping(path = "/side-banner/{seq}")
    @MenuValidApi(menuCode = "CNT002", operation = CrudOperation.READ)
    public ApiResponseDto<Banner>modify(@PathVariable Long seq, @Validated @RequestBody BannerRegReq modReq, AdminAuth adminAuth) throws ApiException {
        ...
        modReq.setLocationCode(BannerCodes.BANNER_SIDE_LOCATION.getCode());
